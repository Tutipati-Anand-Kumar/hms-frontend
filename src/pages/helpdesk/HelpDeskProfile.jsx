...
import { User, Mail, Phone, Headphones, Edit, Clock } from "lucide-react";
...
const HelpDeskProfile = () => {
  ...
  const userName = profile.name || 'HelpDesk User';
  ...
  return (
    <div className="h-full w-full  md:p-8 overflow-y-auto custom-scrollbar" style={{ backgroundColor: 'var(--bg-color)' }}>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {/* Compact Header */}
        <div className="rounded-2xl p-6 border shadow-xl flex flex-col md:flex-row items-center gap-6" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
          {/* Avatar */}
          <div className="shrink-0">
            {profile.avatar ? (
              ...
              <div className={`w-24 h-24 rounded-full border-4 flex items-center justify-center text-2xl font-bold text-white shadow-lg ${getColor(userName)}`} style={{ borderColor: 'var(--bg-color)' }}>
          ...
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-sm font-bold mb-1" style={{ color: 'var(--text-color)' }}>
              {userName}
            </h1>
            <div className="flex items-center justify-center md:justify-start gap-3 text-sm" style={{ color: 'var(--secondary-color)' }}>
              <span className="px-2.5 py-0.5 rounded-full bg-orange-500/10 text-orange-500 dark:text-orange-400 border border-orange-500/20 capitalize text-xs font-medium">
                Front Desk
              </span>
          ...
          {/* Edit Button */}
          <button
            onClick={() => navigate('/helpdesk/profile/edit')}
        ...
        {/* Contact Information */}
        <div className="rounded-2xl p-6 border shadow-xl" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
...
const InfoItem = ({ icon: Icon, label, value, color, bgColor, fullWidth }) => (
  <div className={`${fullWidth ? 'md:col-span-2' : ''} flex items-start gap-4 p-3 rounded-xl  dark:hover:bg-gray-800/50 transition-colors`}>
    ...
    <div>
      ...
      <p className="font-medium text-sm md:text-base break-words" style={{ color: 'var(--text-color)' }}>
        {value || <span className="opacity-50 italic">Not provided</span>}
      </p>
    </div>
  </div>
);
